...
| 🗣 **Voice Cloning** | `OpenVoice V2` | Trích xuất đặc trưng giọng nói của từng speaker gốc để sinh ra giọng lồng tiếng Việt tương đương (giữ nguyên âm sắc, giới tính, cảm xúc). |
...
```
             ...
             └─────────────────────────┘
                           ...
                           ▼
             ┌─────────────────────────┐
             │  7. Voice Cloning       │  Sinh giọng nói tiếng Việt (OpenVoice V2)
             └─────────────────────────┘
                           │
                           ▼
             ┌─────────────────────────┐
...
5. Tải về và cài đặt các thư viện đặc thù từ mã nguồn Git của bên thứ ba trong thư mục `third_party/`:
   - **OpenVoice V2** (Dùng cho Voice Cloning)
...
Các mô hình học sâu như `pyannote.audio` (Speaker Detection), `SenseVoice` (ASR), `OpenVoice` (Voice Cloning) và `Wav2Lip` (Lip Sync) ngốn rất nhiều bộ nhớ đồ họa nếu chạy song song. Đối với các card đồ họa phổ thông 8GB VRAM (như RTX 3060, 4060, 5060):
